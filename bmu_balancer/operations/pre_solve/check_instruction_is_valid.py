...
def asset_can_be_assigned_to_boa(
    ...
    if not respects_availability:
        log.warning(f"{asset} could not be used as candidate because not available.")
        return False
    ...
    if not respects_min_zero_time:
        log.warning(f"{asset} could not be used as candidate because does not respect min zero time.")
        return False
    ...
    if not respects_min_non_zero_time:
        log.warning(f"{asset} could not be used as candidate because does not respect min non-zero time.")
        return False

    # If everything passes and the function has got to this stage, then return True.
    log.info(f"{asset} can be assigned to {boa}.")
    return True
